...
double median (int k, int A[]);
double average (int k, int A[]);
...
struct data{
  ...
  int m; //namu darbu rezultatu kiekis
};//struktura

int main ()
{
  data *darray;//dinaminio masyvo pointeris
  int check=1, choice, cchoice;
  int k;//masyvo dydis
  cout << "Iveskite mokiniu kieki \n";
  cin >> k;
  darray = new data[k];
  for (int n=0; n!=k; n++)//jeigu 1, ciklas kartojamas, jeigu ivedamas kitas skaicius, ciklas baigiasi, jeigu cin.fail(), ciklas taip pat baigiasi
  {
    ...
    cout << "Kiek norite ivesti pazymiu? \n";
    cin >> darray[n].m;
    while(cin.fail() || darray[n].m<=0)      //jeigu ne int tipo
        {
            cout << "Ivedete netinkama reiksme " << std::endl;
            cin.clear();
            cin.ignore(256,'\n');
            cin >> darray[n].m;
        }
    darray[n].nd = new int[darray[n].m];
    ...
    if (choice==1)
    {
      ...
      {
          cin >> darray[n].nd[j];
          while (darray[n].nd[j]<0 || darray[n].nd[j]>10 ||cin.fail())
          {
            cin.clear();
