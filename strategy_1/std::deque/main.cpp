...
int main ()
{
  ...
  srand(time(0));
  deque<student> B;//vektorius, kuriame bus kiekvieno mokinio duomenys
  //-----
  ...
  deque<student> M; //i si vektoriu nuskaitys sugeneruota faila
  deque<student> protingi; //du vektoriai, skirti v1.0 versijai
  deque<student> kvaili; //+
 ...
 else if (read=='2')
 {
    ...
    for (int i=0; i<5; i++)//gali ramo neuztekt:D
    {
       ...
       M.shrink_to_fit();
       distributeFile(numOfStudents, M, protingi, kvaili);
       M.clear();
       protingi.clear();
       kvaili.clear();
