...
void distributeFile(int numOfStudents, deque<student> &M, deque<student> &protingi, deque<student> &kvaili)
{
    ...
    int kv=0, prt=0;
    for (deque<student>::iterator i=M.begin(); i!=M.end(); ++i)
    {
        if ((*i).average<5)
        {
            kvaili.push_back((*i));
    ...
    auto start = chrono::steady_clock::now();
    for (deque<student>::iterator i=kvaili.begin(); i!=kvaili.end(); ++i)
    {
        bad.width(15);
        bad << left << (*i).name;
        bad.width(15);
    ...
    for (deque<student>::iterator i=protingi.begin(); i!=protingi.end(); ++i)
    {
        good.width(15);
        good << left << (*i).name;
        good.width(15);
