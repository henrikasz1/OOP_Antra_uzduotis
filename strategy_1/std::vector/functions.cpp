...
void readFile(vector<data> &M, string name)
{
...
void distributeFile(int numOfStudents, vector<data> &M, vector<data> &protingi, vector<data> &kvaili)
{
    ...
    for (vector<data>::iterator i=M.begin(); i!=M.end(); ++i)
    {
        if ((*i).average<5)
        {
            kvaili.push_back((*i));
    ...
    auto start = chrono::steady_clock::now();
    for (vector<data>::iterator i=kvaili.begin(); i!=kvaili.end(); ++i)
    {
        bad.width(15);
        bad << left << (*i).name;
        bad.width(15);
    ...
    for (vector<data>::iterator i=protingi.begin(); i!=protingi.end(); ++i)
    {
        good.width(15);
        good << left << (*i).name;
        good.width(15);
