...
void readFile(list<student> &M, string name, int amount)
{
    ...
    getline(in, line);
    M.resize(amount);
    for (list<student>::iterator k=M.begin(); k!=M.end(); k++)
    {
        if(getline(in, line))
        {
...
void distributeFile(int numOfStudents, list<student> &M, list<student> &kvaili)
{
    ...
    list<student>::iterator it = M.begin();
    while (it != M.end())
    {
        if((*it).average < 5)
        {
    ...
    auto start = chrono::steady_clock::now();
    for (list<student>::iterator i=kvaili.begin(); i!=kvaili.end(); ++i)
    {
        bad.width(15);
        bad << left << (*i).name;
        bad.width(15);
    ...
    for (list<student>::iterator i=M.begin(); i!=M.end(); ++i)
    {
        good.width(15);
        good << left << (*i).name;
        good.width(15);
